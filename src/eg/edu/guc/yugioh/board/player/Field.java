...
public class Field {
	...
	public boolean addMonsterToField(MonsterCard monster, Mode m,
			boolean isHidden,ArrayList<MonsterCard> sacrifices) {

		if (!(hand.contains(monster) && monster.getLocation() == Location.HAND))
			return false;

		if (monstersArea.size()-sacrifices.size() >= 5)
			throw new NoMonsterSpaceException();
	...
	public boolean addMonsterToField(MonsterCard monster, Mode m,
		...
		boolean monsterAdded = addMonsterToField(monster, m, hidden,sacrifices);
